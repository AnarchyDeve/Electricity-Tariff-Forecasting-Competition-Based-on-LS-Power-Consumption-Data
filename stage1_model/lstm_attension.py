# ==========================================================
# ⚡ stage1_model_train.py
# ✅ PyTorch + MLflow (Stage1 전처리모델 → stage1_model 폴더 저장)
# ==========================================================
...
# ==========================================================
# 데이터 경로 (상위 폴더 기준으로 자동 인식)
# ==========================================================
import os

BASE_DIR = os.path.dirname(os.path.abspath(__file__))   # 현재 폴더 경로
DATA_DIR = os.path.join(BASE_DIR, "../data")            # 상위폴더/data
...
BASE_DIR = "stage1_model"   # ✅ 사용자가 직접 만든 폴더
...
def train_one_fold(model, train_loader, val_loader, optimizer, scheduler, criterion, epochs, fold_name):
    ...
    for epoch in range(epochs):
        ...
        mlflow.log_metrics({f"{fold_name}_train": train_loss, f"{fold_name}_val": val_loss}, step=epoch)
...
def make_objective(target):
    ...
    def objective(trial):
        ...
        with mlflow.start_run(run_name=f"{target}_run"):
